...
# Reading CSV File
df = pd.read_csv(fileOne, delimiter=dlmtr, index_col=None, low_memory=False)
df1 = pd.read_csv(fileTwo, delimiter=dlmtr, index_col=None, low_memory=False)
...
# Finding Extra Rows
s3 = l1.loc[~index1.isin(index2)]
s4 = l2.loc[~index2.isin(index1)]
...
print("Please Refer :\nFor Row level value : Row_Output.csv  \nFor Extra Row value : Extra_Row_Output.csv \nFor "
      "Column level value : Col_output.csv")
